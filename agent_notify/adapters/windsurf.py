...
Windsurf 在 Agent 模式下等待用户确认时，窗口标题包含 "needs attention"。
...
def _get_windsurf_window_titles() -> list[str]:
    """枚举所有窗口标题，返回含 'Windsurf' 且含 'needs attention' 的标题。"""
    titles: list[str] = []
    ...
    def _enum_callback(hwnd, _):
        ...
        if length > 0:
            ...
            title = buf.value
            if "Windsurf" in title and "needs attention" in title.lower():
                titles.append(title)
        return True
...
@register_adapter
class WindsurfAdapter(PollingAdapter):
    ...
    def check(self) -> None:
        """检测 Windsurf 窗口标题，状态变化时发射事件。"""
        try:
            titles = _get_windsurf_window_titles()
        except Exception:
            return
