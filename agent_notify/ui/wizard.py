...
# ── 样式常量 ──
STYLE_WELCOME_TITLE = 'font-size: 22px; font-weight: bold; color: #e0e0e0; margin-bottom: 8px;'
    ...
    def initializePage(self):
        ...
        for key, installed in self._detected.items():
            ...
            status.setStyleSheet(
                'font-size: 11px; color: #4ecf6e;' if installed else 'font-size: 11px; color: #606060;'
            )
...
class SelectAgentsPage(QWizardPage):
    ...
    def _setup_ui(self):
        ...
        group = QGroupBox()
        group.setStyleSheet('QGroupBox { border: 1px solid #404040; border-radius: 6px; padding: 10px; }')
...
class HookConfigPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        if not claude_selected:
            ...
            self._other_label.setText('其他 Agent 无需额外配置，会自动通过信号文件接收通知。')
...
class AutostartPage(QWizardPage):
    ...
    def _setup_ui(self):
        ...
        hint = QLabel('建议开启，这样你每次开机后 Agent Notify 会自动运行，无需手动打开。')
        hint.setStyleSheet(STYLE_SUBTITLE)
        hint.setWordWrap(True)
        layout.addWidget(hint)
