...
API 密钥已配置在项目根目录的 `.env` 文件中。使用前先加载：

```bash
source .env
# 或在 PowerShell 中：
# $env:NUTRIENT_API_KEY = (Get-Content .env | Where-Object { $_ -match '^NUTRIENT_API_KEY=' } -Raw).Split('=',2)[1]
```

如需自行注册新密钥：**[nutrient.io](https://dashboard.nutrient.io/sign_up/?product=processor)**
